...
class Formatter:
    ...
    def link_format(self) -> Format:
        ...
        """
        return Format(
            colon=ENGLISH.format_colon,
            comma=ENGLISH.format_comma,
            dash=ENGLISH.format_dash,
            semicolon=ENGLISH.format_semicolon,
            book_only=False,
            number_only=False,
            property="abbrev",
        )
    ...
    def name_format(self, language: Language) -> Format:
        return Format(
            colon=language.format_colon,
            comma=language.format_comma,
            dash=language.format_dash,
            semicolon=language.format_semicolon,
            book_only=False,
            number_only=False,
            property="name",
        )
    ...
    def make_inter_book_range(self, _: Range, format: Format) -> str:
        ...
        if _.start.verse == 1 and _.end.verse == 999:
            ...
            start_number = string_together(
                (_.start.chapter, format.colon, _.start.verse)
            )
        end_book = self.books[_.end.library, _.end.book]
        end_book_name = getattr(end_book, format.property or "", "")
        if _.start.verse == 1 and _.end.verse == 999:
